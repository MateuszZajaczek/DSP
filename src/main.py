...
def K(N): 
    k = np.arange(N).reshape(-1,1); mu = np.arange(N).reshape(1,-1)
    return k @ mu
...
def main():
    ...
    ap.add_argument("--pdf_name", default="Mateusz_Zajaczek_Variant1_IDFT_in_Matrix_Notation.pdf")
    args = ap.parse_args()
    ...
    xmu = variants[args.variant]; N = xmu.size
    ...
    plt.figure(); plt.stem(range(N), np.real(xk)); plt.title(f"Re{{x[k]}} (N={N})")
    plt.xlabel("k"); plt.ylabel("Re{x[k]}"); plt.savefig(re_png, dpi=150, bbox_inches="tight"); plt.close()
    plt.figure(); plt.stem(range(N), np.imag(xk)); plt.title(f"Im{{x[k]}} (N={N})")
    plt.xlabel("k"); plt.ylabel("Im{x[k]}"); plt.savefig(im_png, dpi=150, bbox_inches="tight"); plt.close()

    styles = getSampleStyleSheet()
    mono = ParagraphStyle(name="Mono", parent=styles["BodyText"], fontName="Courier", fontSize=8.5, leading=10.5, alignment=TA_LEFT)
    doc = SimpleDocTemplate(str(args.out/args.pdf_name), pagesize=A4, rightMargin=1.2*cm, leftMargin=1.2*cm, topMargin=1.2*cm, bottomMargin=1.2*cm)
    story = []
    story += [
        Paragraph("Mateusz Zajaczek – Variant 1: IDFT in Matrix Notation", styles["Title"]),
        Spacer(1,0.3*cm),
